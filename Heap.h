...
class Heap {
    private:
        ...
        void fixDown(int pos) {
            while((2*pos+1) < arr.getSize()) {
                int elem = arr.getElement(pos);
                int leftPos = 2*pos + 1;
                int rightPos = 2*pos + 2;

                if(rightPos >= arr.getSize()) {
    ...
    public:
        ...
        void addElement(T key, int pos) {
            ...
            arr.addElement(pos, key);
            if(pos == arr.getSize() - 1) {
               fixUp(pos);
            } else {
                for(int i = pos ; i < arr.getSize() ; i++) {
                    fixUp(i);
                }
            }
        }
        ...
        int deleteElementByPos(int pos, T key = 0) { // with option to check key
            if(pos >= 0 && pos < arr.getSize()) {
                if(arr.getElement(pos) == key || key == 0) {
                    if(pos == arr.getSize()-1) {
                       arr.deleteElementByPos(pos);
                       return 1;
                    }
                    T lastElem = arr.getElement(arr.getSize()-1);
                    arr.swapElements(pos, arr.getSize()-1);
                    arr.deleteElementByPos(arr.getSize()-1);
        ...
        void fillRandom(int newSize, int minVal = 1, int maxVal = 100) { // przedzial do 100, nie do INT_MAX, w celu zmniejszenia rozmiaru danych(drukowanie drzewa sie nie rozjezdza)
            arr.setSize(0);
